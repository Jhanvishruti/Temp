...
<p className="text-sm">{owner.proDes}</p>

<div className="flex items-center gap-2">

<Clock className="h-4 w-4" />

<span>{`${owner.timeNeedValue} ${owner.timeUnit}`}</span>
...
{owner.reqSkills?.split(',').map((skill, index) => (
...
<p>Type: {owner.proType}</p>

<p>Hours per day: {owner.hoursPerDay}</p>

<p>Collaboration: {owner.collabMode}</p>
